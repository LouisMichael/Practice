...
# game plan is to store curState, rates and costs in the order [ore,clay,obsidian, gem]
import re
matarialTypes = ["ore","clay","obsidian","geode"]
memo = {}
def addBasedOnCurBots(curAvailable, curBots):
    for i, bot in enumerate(curBots):
        curAvailable[i] += curBots[i]
    curAvailable[4] -= 1
def subBasedOnCurBots(curAvailable, curBots):
    for i, bot in enumerate(curBots):
        curAvailable[i] -= curBots[i]
    curAvailable[4] += 1
costMax = []
def findMaxGeod(bluePrintInput, curAvailable, curBots):
    # try to buy each type of bot
    # print(bluePrintInput)
    # print(curAvailable)
    # print(curBots)
    max = 0
    # if str(curBots) + str(curAvailable) in memo:
    #     return memo[str(curBots)+str(curAvailable)]
    if curAvailable[4] <= 0:
        return curAvailable[3]
    for l, botCost in reversed(list(enumerate(bluePrintInput))):
        canBuildBot = True
        for k in range(3):
            if botCost[k] > curAvailable[k]:
                # print("it cost " + str(materialCost) + " : " + matarialTypes[i] + " but we have " + str(curAvailable))
                canBuildBot = False
                continue
        if canBuildBot and (curBots[l] < costMax[l] or l == 3):
            # print("can build " + str(botCost))
            for f, materialCost in enumerate(botCost):
                curAvailable[f] -= materialCost
            addBasedOnCurBots(curAvailable, curBots)
            curBots[l] += 1
            buyBotResult = findMaxGeod(bluePrintInput, curAvailable, curBots)
            curBots[l] -= 1
            subBasedOnCurBots(curAvailable, curBots)
            for f, materialCost in enumerate(botCost):
                curAvailable[f] += materialCost
            if buyBotResult> max:
                max = buyBotResult
            if l == 3:
                # print("can build gem bot")
                break
    addBasedOnCurBots(curAvailable, curBots) 
    noBuyResult = findMaxGeod(bluePrintInput, curAvailable, curBots)
    subBasedOnCurBots(curAvailable, curBots)
    if noBuyResult > max:
        max = noBuyResult
    # memo[str(curBots)+str(curAvailable)] = max
    return max
...
with open("test.txt", "r", encoding="utf-8") as f:
    ...
    for line in lines:
        line = line.strip()
        bluePrint = []
        for material in matarialTypes:
            botCost = []
            regex = "Each "+ material + " robot costs ([^.]*)."
            costAsWords = re.search(regex, line)[1]
            for materialType in matarialTypes:
                regex2 = "(\d*) " + materialType
                # print(regex2)
                searchResult = re.search(regex2, costAsWords)
                # print(searchResult)
                if searchResult:
                    botCost.append(int(searchResult[1]))
                else:
                    botCost.append(0)
            bluePrint.append(botCost)
        # print(bluePrint)
    ...
    for i, bluePrint in enumerate(bluePrints):
        ...
        for a in bluePrint:
            for j, c in enumerate(a):
                if c > costMax[j]:
                    costMax[j] = c
        bluePrintMax = findMaxGeod(bluePrint,[0,0,0,0,24],[1,0,0,0])
        print(bluePrintMax)
        solve += (i+1) * bluePrintMax
        memo = {}
    print(solve)
